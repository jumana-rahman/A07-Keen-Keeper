    ...
    return (
        <div>
            <div className='card text-center px-6 py-8 bg-white border border-gray-100 rounded-sm'> 
                ...
                <h3 className='font-semibold text-[20px] text-[#1F2937]'>{friend.name}</h3> 
                
                <p className='text-[12px] text-[#64748B] pb-2'>{friend.days_since_contact}d ago</p> 

                <div className='flex justify-center mt-1.5'> 
                    <div className={`badge text-[12px] text-white uppercase rounded-full ${getStatusColor(friend.status)}`}>{friend.status}</div> 
                </div> 
                ...
                <div className='flex justify-center gap-1 mt-2'> 
                    
                    {
                        friend.tags.map((tag, index) => (
                            <div key={index} className="badge bg-[#CBFADB] text-[12px] text-[#244D3F] uppercase rounded-full">{tag}</div> 
                        ))
                    }
                     
                </div> 

                <p className='text-[#64748B] text-[16px] font-medium italic max-w-63.5 mx-auto pt-5'> {friend.bio} </p>
                
                <p className='text-[#64748B] text-[14px] max-w-63.5 mx-auto pt-2'> {friend.email} </p>
